...
// Post struct
type Post struct {
	PostID    string             `bson:"_id,omitempty" json:"post_id"`
...
func registerSongHandler(w http.ResponseWriter, r *http.Request) {
	...
	song1.Popularity = strconv.Itoa(song.Popularity) //string(song.Popularity)
...
func httpHandler() http.Handler {
	...
	// Delete Post route
	router.HandleFunc("/deletePost/{post_id}", deletePost).Methods("DELETE")
...
func addPostHandlerForTesting(w http.ResponseWriter, r *http.Request, client *mongo.Client, postCollection *mongo.Collection) {
	...
	if err != nil {
		http.Error(w, "Error posting post", http.StatusInternalServerError)
		return
	}
...
func deletePostHandlerForTesting(w http.ResponseWriter, r *http.Request, client *mongo.Client, postCollection *mongo.Collection) {
	...
	result, err := postCollection.DeleteOne(context.TODO(), bson.M{"_id": requestBody.PostID})
	if err != nil {
...
func updatePostHandlerForTesting(w http.ResponseWriter, r *http.Request, client *mongo.Client, postCollection *mongo.Collection) {
	var requestBody struct {
		PostID  string `json:"postID"`
	...
	if requestBody.Content != "" {
		updateFields["content"] = requestBody.Content
	}
	if requestBody.Link != "" {
		updateFields["link"] = requestBody.Link
	}
